...
client.on("channelDelete", async channel => {
  ...
  if(deleter.id == "758946334702829599") return; //bu satıra kendi id'nizi yazın sizin kanal silmenizi engellemeyecektir
...
if(client.ping > 250) {
           ...
           let sChannel = msg.guild.channels.find(c => c.name === "sohb22et")
...
client.on("guildBanAdd", async (guild, user) => {
  ...
  if (yetkili.id === "598123594790207499") return;    ///sag tık atarsa bir şey yapılmayacak ıd
...
if(channel.guild.id !== "697751599375974431") return;   //Sunucu ıd
    ...
    if(logs.entries.first().executor.bot) return;
      channel.guild.member(logs.entries.first().executor).roles.filter(role => role.name !== "@everyone").array().forEach(role => {
              channel.guild.member(logs.entries.first().executor).removeRole(channel.guild.roles.get("697754351153971260")) //alınacak rol ıdler
              channel.guild.member(logs.entries.first().executor).removeRole(channel.guild.roles.get("697754351552299019"))
    })
const sChannel = channel.guild.channels.find(c=> c.id ==="697754540191252532")   //log ıd
const cıks = new Discord.RichEmbed()
.setColor('RANDOM')
.setDescription(`${channel.name} adlı Kanal silindi Silen kişinin yetkilerini aldım :dragon:`)
.setFooter('CenkElmurza')
sChannel.send(cıks)
  
channel.guild.owner.send(` **${channel.name}** adlı Kanal silindi Silen  kişinin yetkilerini aldım:dragon:`)
}) 
////
///////////////////
///////////////////client.on('guildMemberAdd',async member => {
